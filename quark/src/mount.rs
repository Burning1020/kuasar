...
pub async fn bind_mount<T: AsRef<Path>>(source: T, target: &str, options: &[String]) -> Result<()> {
    let target = Path::new(target);
    if !target.exists() {
        if !source.as_ref().is_dir() {
            debug!(
                "create a file {} because source {} is a file",
                target.display(),
            ...
            debug!(
                "create a directory {} because source {} is a dir",
                target.display(),
                source.as_ref().display()
            );
    ...
    .map_err(|e| anyhow!("failed to mount {}, {}", target.display(), e))?;
    ...
    if flags.contains(MsFlags::MS_RDONLY) {
        ...
        .map_err(|e| anyhow!("failed to mount {} as read only, {}", target.display(), e))?;
    }
    Ok(())
}
